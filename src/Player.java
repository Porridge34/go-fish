...
public class Player {
    ...
    private static List<Card> cpucardlist = new ArrayList<Card>();
    public Player() {
    ...
    public Card getCardByNeed() {
        ...
        for (int i = 0; i < hand.size() - 1; i++) {
            ...
            if (count > frequency) {// updates which card is the most frequently occurring
                boolean found = false;
                if (cpucardlist != null) {
                    for (int k = 0; k < cpucardlist.size(); k++) {
                        if (cpucardlist.get(k).getRank().equals(hand.get(i).getRank())) {
                            found = true;
                        }
                    }
                }
                if (found == false) {
                    cpucardlist.add(hand.get(i));
                    index = i;
                    frequency = count;
                    if (frequency < 2) {
                        emptycpulist();
